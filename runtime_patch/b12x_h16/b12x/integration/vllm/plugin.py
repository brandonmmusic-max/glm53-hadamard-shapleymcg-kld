...
def register_b12x_fp6() -> None:
    ...
    class _VllmMoEMethod(FusedMoEMethodBase):  # type: ignore[misc]
        ...
        def __init__(self, moe_config: Any, source_format: str):
            ...
            if os.environ.get("GLM53_MXFP6_H16_ALL", "").strip().lower() not in {
                "1", "true", "yes", "on"
            }:
                raise RuntimeError(
                    "the dedicated MXFP6 H16 runtime requires "
                    "GLM53_MXFP6_H16_ALL=1"
                )
            self.source_format = source_format
            self.core: Optional[B12XFP6MoEMethod] = None
            self._out_bufs: dict[int, torch.Tensor] = {}
            self._glm53_h16: Optional[torch.Tensor] = None
            self._glm53_h16_logged = False
        ...
        def process_weights_after_loading(self, layer: Any) -> None:
            ...
            h = torch.ones((1, 1), dtype=torch.bfloat16, device=dev)
            while h.shape[0] < 16:
                h = torch.cat(
        ...
        def apply(
            ...
            if x.shape[-1] % 16:
                raise RuntimeError("GLM53 MXFP6 H16 input width is not divisible by 16")
            if (
                self._glm53_h16 is None
                or self._glm53_h16.device != x.device
                or self._glm53_h16.dtype != x.dtype
            ):
                h = torch.ones((1, 1), dtype=x.dtype, device=x.device)
                while h.shape[0] < 16:
                    h = torch.cat(
                        (torch.cat((h, h), dim=1), torch.cat((h, -h), dim=1)),
                        dim=0,
                ...
                self._glm53_h16 = h / 4
            x = (x.reshape(*x.shape[:-1], -1, 16) @ self._glm53_h16).reshape_as(x)
            if not self._glm53_h16_logged:
                print(
        ...
        def get_quant_method(self, layer: Any, prefix: str):
            ...
            if is_moe:
                if self._has_fp6_experts(prefix):
                    ...
                    method = _VllmMoEMethod(layer.moe_config, self._source_format)
                    method._glm53_prefix = prefix
                    return method
