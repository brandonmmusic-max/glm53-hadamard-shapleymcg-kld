...
ROOT="$CAMPAIGN/codec-v2/output-aware/gptq-control-layer${LAYER}"
...
for slot in 0 1 2; do
  ...
  printf -v E3 '%03d' "$end"
  CUDA_VISIBLE_DEVICES=$gpu PYTHONPATH=. python3 -m glm53_nvfp4.quantize_layer \
    --source "$SOURCE" --source-index "$INDEX" --capture-root "$CAPTURE" \
    --roles "$ROLES" --layer "$LAYER" --expert-start "$start" --expert-end "$end" \
    ...
    --output "$ROOT/chunks/gptq-layer-$L3-experts-$S3-$E3.safetensors" \
    --receipt "$ROOT/receipts/gptq-layer-$L3-experts-$S3-$E3.json" \
