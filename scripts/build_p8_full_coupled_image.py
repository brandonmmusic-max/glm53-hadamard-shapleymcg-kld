#!/usr/bin/env python3
"""Prepare or explicitly build the immutable v10 full-coupled P8 research image.

v10 differs from v9 only in the sitecustomize design-hash allowlist, so identity
and coupled sidecars encoded under different pinned designs can load together.
"""
...
DOCKERFILE = REPO / "runtime_patch/p8_full_coupled_image/Dockerfile"
MANIFEST = REPO / "runtime_patch/p8_full_coupled_image/image_manifest.json"
...
TAG = "klc/glm53-p8-full-coupled:v10"
...
def load_manifest() -> dict[str, object]:
    ...
    if manifest["schema"] != "glm53.p8-coupled-image-sources.v10":
...
def build_command(
    *, context: Path, source_commit: str, source_tree: str, tag: str
    ...
    return [
        ...
        "--network=none",
        "--file",
        str(context / "runtime_patch/p8_full_coupled_image/Dockerfile"),
        "--build-arg",
    ...
    *, output_dir: Path, manifest: dict[str, object]
    ...
    relative_paths = [
        *manifest["source_sha256"],
        "runtime_patch/p8_full_coupled_image/image_manifest.json",
    ]
...
def preflight(*, output_dir: Path, source_commit: str, tag: str) -> dict[str, object]:
    ...
    source_hashes["runtime_patch/p8_full_coupled_image/image_manifest.json"] = sha256(MANIFEST)
    source_hashes["scripts/build_p8_full_coupled_image.py"] = sha256(Path(__file__))
...
def execute(plan: dict[str, object], output_dir: Path) -> None:
    ...
    required_labels = {
        "org.klc.experiment": "glm53-p8-full-coupled-h512-h128-suh-svh-v10",
