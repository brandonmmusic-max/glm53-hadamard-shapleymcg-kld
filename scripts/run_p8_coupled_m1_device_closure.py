#!/usr/bin/env python3
"""Opt-in SM120 M1 closure for the full-coupled P8 native wrapper.
...
MODULE_SOURCES = {
    "p8_native_kernel": "runtime_patch/p8_native_kernel.py",
    ...
    "b12x.moe._shared.kernels.p8_coupled_topk": (
        "runtime_patch/b12x_h16/b12x/moe/_shared/kernels/p8_coupled_topk.py"
    ),
}
PROTOCOL = {
    "schema": "glm53.p8-full-coupled-m1-device-closure.v1",
    ...
    "exact_gates": [
        ...
        "K4 trellis and selected UE8M0 weight bytes decode as finite E4M3",
    ...
    "claim_boundary": (
        "one synthetic layer-3/rank-0 M1 TP-local device closure; not KLD, "
...
def _load_sidecar_reference(path: Path, transform_sha256: str):
    import torch
...
def _decode_expert(path: Path, expert: int):
    import torch
    from safetensors import safe_open
    ...
    if tuple(w13.shape) != (2, 256, 32, 64) or tuple(w2.shape) != (32, 256, 64):
        raise RuntimeError(f"selected K4 trellis geometry differs for expert {expert}")
    ...
    if w13.dtype != torch.int16 or w2.dtype != torch.int16:
        raise RuntimeError("K4 trellis stream must use packed int16 words")
    ...
    gate = decode_trellis_mxf(
        w13[0], lut, w13_scale[512:], bits=4, block_size=32,
        rows=512, width=4096, device="cpu",
    )
    up = decode_trellis_mxf(
        w13[1], lut, w13_scale[:512], bits=4, block_size=32,
        rows=512, width=4096, device="cpu",
    )
    down = decode_trellis_mxf(
        w2, lut, w2_scale, bits=4, block_size=32,
...
def _reference(path: Path, x, weights, scales):
    ...
    for slot, expert in enumerate(EXPERT_IDS):
        gate, up, down, receipt = _decode_expert(path, expert)
...
def run_probe(args: argparse.Namespace) -> dict[str, object]:
    ...
    reference = _reference(args.sidecar, x_cpu, weights_cpu, scales)
...
def outer_execute(args: argparse.Namespace) -> None:
    ...
    command = [
        ...
        "/work/scripts/run_p8_coupled_m1_device_closure.py", "--probe",
        "--image-id", args.image,
    ...
    launch = {
        "schema": "glm53.p8-full-coupled-m1-device-launch.v1",
        "protocol_sha256": PROTOCOL_SHA256,
        "image_id": args.image,
