#!/usr/bin/env python3
"""Prepare or explicitly build the immutable v11 mixed-rate P8 research image.

v11 adds per-layer K3/K4/K5 trellis rates to the small-M coupled owner path
(bit-parameterized FC1/FC2 kernels, K5-capable procedural MCG decoder) on top of
the v10 design-hash allowlist.  Grouped M64 prefill stays K4-only; non-K4 layers
serve M>1 row by row through the exact M1 kernel.
"""
...
DOCKERFILE = REPO / "runtime_patch/p8_mixed_rate_image/Dockerfile"
MANIFEST = REPO / "runtime_patch/p8_mixed_rate_image/image_manifest.json"
...
def load_manifest() -> dict[str, object]:
    ...
    if manifest["schema"] != "glm53.p8-coupled-image-sources.v11":
    ...
    if manifest["integration_base_commit"] != INTEGRATION_BASE:
        raise ValueError("manifest integration base differs from the repaired runtime")
...
def build_command(
    *, context: Path, source_commit: str, source_tree: str, tag: str
    ...
    return [
        ...
        "--network=none",
        "--file",
        str(context / "runtime_patch/p8_mixed_rate_image/Dockerfile"),
        "--build-arg",
...
def preflight(*, output_dir: Path, source_commit: str, tag: str) -> dict[str, object]:
    ...
    source_hashes["runtime_patch/p8_mixed_rate_image/image_manifest.json"] = sha256(MANIFEST)
    source_hashes["scripts/build_p8_mixed_rate_image.py"] = sha256(Path(__file__))
...
def execute(plan: dict[str, object], output_dir: Path) -> None:
    ...
    required_labels = {
        "org.klc.experiment": "glm53-p8-mixed-rate-coupled-k3k4k5-v11",
