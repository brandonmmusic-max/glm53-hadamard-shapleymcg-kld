"""CPU-only fixed layer3 pack plus source-exact postwrite verification."""
import json
import os
...
def main():
    ...
    plan_path = ROOT / 'experiments/p8-coupled-layer3-pack-v1.json'
    ...
    source_receipts = [validate_chunk(a,b) for a,b in plan['source_ranges']]
    ...
    target = OUT / 'sidecars/layer-003'
    assert not target.exists(), 'preserve existing outputs'
    pack_receipt = OUT / 'receipts/layer-003-sidecars.json'
    verify_receipt = OUT / 'receipts/layer-003-postwrite.json'
    assert not pack_receipt.exists() and not verify_receipt.exists()
    chunk_args = [item for a,b in plan['source_ranges'] for item in ['--chunk',str(paths(a,b)[0])]]
    pack = [sys.executable,'-u','-m','glm53_nvfp4.build_p8_coupled_scale_tp4_sidecars',*chunk_args,'--output-dir',str(target),'--receipt',str(pack_receipt),'--layer','3','--world-size','4']
    sidecar_args = [item for r in range(4) for item in ['--sidecar',str(target / f'p8-layer-003-tp4-rank-{r}.safetensors')]]
    verify = [sys.executable,'-u','-m','glm53_nvfp4.verify_p8_coupled_scale_tp4_sidecars',*chunk_args,*sidecar_args,'--packer-receipt',str(pack_receipt),'--receipt',str(verify_receipt),'--layer','3']
