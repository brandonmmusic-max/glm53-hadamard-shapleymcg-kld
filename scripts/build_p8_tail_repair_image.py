#!/usr/bin/env python3
"""Build the CPU-only, capture-capable P8 KPool-tail repair image."""
...
CONTEXT = REPO / "runtime_patch/p8_tail_repair"
...
SOURCES = ("Dockerfile", "kpool-tail-consumed-v1.patch")
...
def main() -> None:
    ...
    if not args.tag.startswith("klc/glm53-p8-tail-repair:"):
        raise ValueError("dedicated tail-repair tag required")
    if subprocess.run(["docker", "image", "inspect", args.tag], capture_output=True).returncode == 0:
        raise ValueError("tail-repair image tag already exists")
    ...
    receipt = {
        "schema": "glm53.p8-tail-repair-image.v1",
        "status": "started",
        "started_unix_ns": time.time_ns(),
        "parent_image_id": PARENT,
        "original_kpool_sha256": ORIGINAL,
        "tag": args.tag,
    ...
    try:
        ...
        if image_id(PARENT) != PARENT:
            raise ValueError("parent image changed during build")
        line = output([
        ...
        if installed != TARGET or patched_sha == ORIGINAL:
            raise ValueError("tail-repair source was not installed")
        marker = output([
            "docker", "run", "--rm", "--network=none", "--runtime=runc",
            "-e", "NVIDIA_VISIBLE_DEVICES=void", "--entrypoint", "grep",
            built, "-c", "P8 tail-consumed-v1", TARGET,
        ])
        if marker != "1":
            raise ValueError("tail-repair source marker differs")
