"""Analyze the lossless K4 endpoint optimizer screen."""
...
ARMS = ("identity-gptq", "down-h16-gptq", "identity-k4-down-h16")
...
def main() -> None:
    ...
    candidate = np.asarray(output["identity-k4-down-h16"])
    ...
    for control, threshold in (("identity-gptq", 0.05), ("down-h16-gptq", 0.01)):
        ...
        improvement, wins = 1.0 - _gmean(candidate) / _gmean(values), int((candidate < values).sum())
        comparisons[control] = {"relative_improvement": improvement, "wins": wins, "mean_log_ratio_ci95_bca": interval}
        passed &= improvement >= threshold and wins >= 12 and interval[1] < 0
    payload = {
        "schema": "glm53-identity-k4-down-h16-screen-analysis.v1",
        "plan": {"path": str(args.plan), "sha256": plan_sha},
        "inputs": inputs,
        "experts": experts,
        "down_weight_geometric_mean_nmse": {arm: _gmean(v) for arm, v in weight.items()},
        "routed_output_geometric_mean_nmse": {arm: _gmean(v) for arm, v in output.items()},
        "comparisons": comparisons,
        "decision_rule": plan["decision_rule"],
        "decision": "pass-build-full-layer-identity-k4" if passed else "fail-do-not-open-new-teacher-role",
