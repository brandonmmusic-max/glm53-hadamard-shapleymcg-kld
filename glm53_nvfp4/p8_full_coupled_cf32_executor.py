...
def authenticate_runtime_manifest(path: Path, *, docker_inspect=None) -> tuple[dict, list[dict]]:
    ...
    if (value.get("schema") != MANIFEST_SCHEMA or value.get("status") != "sealed-before-execution"
            ...
            or (value.get("runtime") or {}).get("attention_backend") not in runtime.ATTENTION_BACKENDS
            or (value["runtime"]["attention_backend"], value["runtime"]["kv_dtype"]) not in runtime.SUPPORTED_ATTENTION_KV
            or value.get("runtime") != {**RUNTIME_LABELS, "kv_dtype": value["runtime"]["kv_dtype"],
                                        "attention_backend": value["runtime"]["attention_backend"]}):
...
def audit_runtime_log(text: str, arm: str, *, design_by_layer: dict[int, str], image_id: str, bpw: float,
                      ...
                      kv_dtype: str = "nvfp4_ds_mla",
                      attention_backend: str = "B12X_MLA_SPARSE") -> dict:
    if kv_dtype not in runtime.KV_DTYPES:
        raise ValueError(f"undeclared KV-cache dtype: {kv_dtype!r}")
    if (attention_backend, kv_dtype) not in runtime.SUPPORTED_ATTENTION_KV:
        raise ValueError(f"unsupported attention/KV pair: {attention_backend} with {kv_dtype}")
    ...
    conditions = {"attention": attention_backend, "kv_dtype": kv_dtype,
        ...
        for index, window in enumerate(windows):
            ...
            if index == 0:
                ...
                _save(runtime_path, audit_runtime_log(proof_log, arm, kv_dtype=manifest["runtime"]["kv_dtype"],
                                                      attention_backend=manifest["runtime"]["attention_backend"],
