...
def scale_tensors(layer: int, maxima: torch.Tensor) -> dict[str, torch.Tensor]:
    if tuple(maxima.shape) != (288,):
        raise ValueError(f"expected 288 expert maxima, got {tuple(maxima.shape)}")
    result: dict[str, torch.Tensor] = {}
    for expert, amax in enumerate(maxima):
        scale = (amax.float() / NVFP4_GLOBAL_DENOMINATOR).reshape(())
        base = f"model.language_model.layers.{layer}.mlp.experts.{expert}"
        result[f"{base}.gate_proj.input_scale"] = scale
        result[f"{base}.up_proj.input_scale"] = scale.clone()
    return result
...
def main() -> None:
    ...
    capture = LayerCapture(args.capture_root, args.layer, args.roles, max_samples=256)
    ...
    maxima = routed_amax(capture, rotation, device=device)
    tensors = scale_tensors(args.layer, maxima)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    save_file(
        tensors,
        str(args.output),
        metadata={
            "schema": "glm53-nvfp4-v4.rotated-input-scales.v1",
    ...
    receipt = {
        "schema": "glm53-nvfp4-v4.rotated-input-scale-receipt.v1",
        "layer": args.layer,
        "rotation": args.rotation,
