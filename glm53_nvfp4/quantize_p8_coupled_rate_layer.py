"""Encode one GLM-5.3-Flash layer in the coupled H512/H128 P8 basis at K3, K4 or K5.

Mixed-rate sibling of ``quantize_p8_coupled_scale_layer``: identical transform,
scales, calibration and GPTQ-style feedback; only the stored procedural MCG
trellis rate (``--bits``) varies.  Weight payload is exactly ``bits + 0.25`` bpw.
"""
...
def _quantize(weight: torch.Tensor, hessian: torch.Tensor, *, bits: int):
...
def _validate_design(
    ...
    if (
        design.get("schema") != "glm53-p8.coupled-rate-preparation.v3"
        ...
        or design.get("bits") != args.bits
        or design.get("weight_payload_bpw") != args.bits + 0.25
...
def main() -> None:
    ...
    for expert in range(args.expert_start, args.expert_end):
        ...
        payloads = {
            "gate_proj": _quantize(transformed[0], hidden_hessian, bits=args.bits),
            "up_proj": _quantize(transformed[1], hidden_hessian, bits=args.bits),
        }
        ...
        payloads["down_proj"] = _quantize(
            transformed[2], route_weighted_hessian(down_carrier, route), bits=args.bits
        )
    ...
    if not math.isclose(weight_payload_bpw, args.bits + 0.25, rel_tol=0.0, abs_tol=1e-12):
        raise RuntimeError(f"weight payload is {weight_payload_bpw} bpw, expected {args.bits + 0.25}")
    metadata_bytes = sum(
