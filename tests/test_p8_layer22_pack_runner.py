...
def fixture(monkeypatch, tmp_path):
    ...
    spec = importlib.util.spec_from_file_location('pack_runner', root / 'scripts/run_p8_layer22_pack.py')
    ...
    plan = dict(layer=22,campaign_aggregate_bound_bytes=29873009168,user_ceiling_bytes=30000000000,existing_external_bound_bytes=6573009168,packer_sha256='pin',verifier_sha256='pin',source_ranges=[[0,72],[72,144],[144,216],[216,288]],layer_artifact_bound_bytes=23300000000)
    (module.ROOT / 'experiments/p8-coupled-layer22-pack-v1.json').write_text(json.dumps(plan))
...
def test_failed_packer_does_not_run_verifier(monkeypatch,tmp_path):
    ...
    assert len(calls) == 1
    assert json.loads((module.OUT/'layer22-pack.execution.json').read_text())['exit_code'] == 9
    assert not (module.OUT/'layer22-postwrite.launch.json').exists()
...
def test_existing_target_is_preserved(monkeypatch,tmp_path):
    module = fixture(monkeypatch,tmp_path)
    target = module.OUT/'sidecars/layer-022'
...
def test_layer22_commands_and_budget_gate(monkeypatch,tmp_path):
    module = fixture(monkeypatch,tmp_path)
    seen = []
    ...
    def launch(command,**kwargs):
        seen.append(command)
        assert command[command.index('--layer')+1] == '22'
        assert kwargs['env']['CUDA_VISIBLE_DEVICES'] == ''
        return SimpleNamespace(returncode=9)
...
def test_over_budget_stops_before_launch(monkeypatch,tmp_path):
    module = fixture(monkeypatch,tmp_path)
    path = module.ROOT/'experiments/p8-coupled-layer22-pack-v1.json'
